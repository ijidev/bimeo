...
class HomeController extends Controller
{
    ...
    public function start()
    {
        ...
        $open_time = new \DateTime( $set->active_hour );
        $close_time = new \DateTime( $set->close_hour );
        $current_time = date('H:i');
        $review = ProductReview::where('user_id',$user->id)
            ->where('status', '!=' , 'approved' )->get();
            //dd(\geoip_time_zone_by_country_and_region());
        ...
        if ($current_time >= $close_time->format('H:i')) 
        {
            return back()->with('error','Active hour passed try again between '. $open_time->format('H:i') . ' & ' . $close_time->format('H:i') . ' current time is:' . $current_time);
        } 

        if ($current_time < $open_time->format('H:i')) 
        {
